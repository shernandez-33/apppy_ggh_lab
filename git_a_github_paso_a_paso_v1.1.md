...
1.  **Establece tu identidad:** Configura tu nombre y correo para que Git sepa quién firma los cambios. Esto es inmutable en el historial.
    ...
    1.1 **Visualizar la configuración global existente**
        El comando más directo para listar todas las variables configuradas en el ámbito global (asociadas a tu usuario del sistema) es:
        ```bash
        git config --global --list
        ```
    1.2 **Ver la ubicación del archivo de configuración**
        Si además de los valores quieres saber exactamente en qué archivo físico se encuentran guardados, utiliza:
        ```bash
        git config --global --list --show-origin
        ```
    1.3 **Consultar un valor específico**
        Si solo te interesa saber un dato en particular (por ejemplo, tu correo), no necesitas listar todo. Puedes preguntar directamente por la clave:
       ```bash
        git config --global user.email
        ```
    1.4 **Abrir el archivo en un editor**
        Si prefieres leer la configuración en un formato más amigable (tipo .ini) o necesitas realizar cambios manuales, puedes abrir el archivo directamente en tu editor de texto predeterminado por consola:
         ```bash
